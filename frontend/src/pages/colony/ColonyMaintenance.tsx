...
const statusColors: Record<RequestStatus, 'default' | 'success' | 'warning' | 'error' | 'info'> = {
  submitted: 'default',
  assigned: 'info',
  in_progress: 'warning',
...
export default function ColonyMaintenance() {
  ...
  const [requestFilter, setRequestFilter] = useState<{ status?: string; category?: string }>({})

  const [requestForm, setRequestForm] = useState<CreateMaintenanceRequest>({
    quarter_number: '',
    category: '',
    sub_category: '',
    description: '',
  ...
  const [assignForm, setAssignForm] = useState<AssignmentPayload>({
    assigned_vendor_id: '',
  ...
  const [statusForm, setStatusForm] = useState<StatusChangePayload>({
    status: 'submitted',
    notes: '',
  ...
  const [vendorForm, setVendorForm] = useState<VendorPayload>({
    name: '',
    company_name: '',
    email: '',
    phone: '',
  ...
  const [technicianForm, setTechnicianForm] = useState<TechnicianPayload>({
    name: '',
    phone: '',
    email: '',
    specialization: '',
  ...
  const [assetForm, setAssetForm] = useState<AssetPayload>({
    asset_number: '',
    asset_type: '',
    quarter_number: '',
    make: '',
  ...
  const [categoryForm, setCategoryForm] = useState<ServiceCategoryPayload>({
    name: '',
    description: '',
    sla_hours: 24,
    icon: '',
  ...
  const [recurringForm, setRecurringForm] = useState<RecurringPayload>({
    name: '',
    description: '',
    category: '',
    frequency: 'monthly',
  ...
  const assignMutation = useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: AssignmentPayload }) =>
      colonyService.assignRequest(id, payload),
    onSuccess: () => {
  ...
  const statusMutation = useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: StatusChangePayload }) =>
      colonyService.changeStatus(id, payload),
    onSuccess: () => {
  ...
  const uploadMutation = useMutation({
    mutationFn: ({ id, file }: { id: string; file: File }) =>
      colonyService.uploadAttachment(id, file),
    onSuccess: () => {
  ...
  const canDecide = (status: RequestStatus) => status === 'submitted'
              ...
              >
                ...
                {Object.keys(statusLabels).map((key) => (
                  <MenuItem key={key} value={key}>
                    {statusLabels[key as RequestStatus]}
                  </MenuItem>
                ))}
              </Select>
            </FormControl>
                ...
                <Stack spacing={2} mb={2}>
                  <FormControl fullWidth size="small">
                    ...
                    <Select
                      ...
                      onChange={(e) => setStatusForm((prev) => ({ ...prev, status: e.target.value as RequestStatus }))}
                    >
                      {Object.keys(statusLabels).map((key) => (
                        <MenuItem key={key} value={key}>{statusLabels[key as RequestStatus]}</MenuItem>
                      ))}
                    </Select>
                  </FormControl>
                  <TextField
