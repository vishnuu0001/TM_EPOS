...
export default function Equipment() {
  ...
  const [maintenanceForm, setMaintenanceForm] = useState<MaintenancePayload>({
    equipment_id: '',
    maintenance_type: '',
    description: '',
    scheduled_date: '',
  ...
  const approveBookingMutation = useMutation({
    mutationFn: (id: string) => equipmentService.approveBooking(id),
    onSuccess: () => {
  ...
  const updateBookingStatusMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: BookingStatus }) =>
      equipmentService.updateBooking(id, { status }),
    onSuccess: () => {
            ...
            <Stack spacing={2}>
              ...
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) => setEquipmentForm((p) => ({ ...p, equipment_type: e.target.value as EquipmentType }))}
                >
                        ...
                        <Stack direction="row" spacing={1}>
                          ...
                          {b.status !== 'COMPLETED' && b.status !== 'CANCELLED' && (
                            <FormControl size="small" sx={{ minWidth: 140 }}>
                              <Select
                                value={b.status}
                                onChange={(e) => updateBookingStatusMutation.mutate({ id: b.id, status: e.target.value as BookingStatus })}
                              >
            ...
            <Stack spacing={2}>
              ...
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) => setCertForm((p) => ({ ...p, equipment_type: e.target.value as EquipmentType }))}
                >
                  {equipmentTypes.map((t) => (
                    <MenuItem key={t} value={t}>{t}</MenuItem>
                  ))}
